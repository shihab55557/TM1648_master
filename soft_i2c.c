...
uint8_t SoftI2C_Read_Register(uint8_t reg_addr){
	SoftI2C_Start();
	SoftI2C_Send_Byte(SoftI2C.WriteAddress);    // Slave address and write bit
	SoftI2C_Send_Byte(reg_addr);                // Register address
	SoftI2C_Stop();
	SoftI2C_Start();
	SoftI2C_Send_Byte(SoftI2C.ReadAddress);     // Slave address and read bit
	uint8_t sts = SoftI2C_Receive_Raw_Byte();   // Receiving data from slave
	SoftI2C_Send_Nack();                        // Sending nack after receiving data
	SoftI2C_Stop();                             // I2C stop after receiving data
	return sts;
}
